...
void sendHtmlResponse(WiFiClient client) {
    ...
    client.println("  fetch('/strip.bin')");
    ...
    client.println("  fetch('/matrix.bin')");
    client.println("    .then(response => response.arrayBuffer())");
    client.println("    .then(buffer => {");
    client.println("      const data = new Uint8Array(buffer);");
    client.println("      const canvas = document.getElementById('matrix-canvas');");
    client.println("      const ctx = canvas.getContext('2d');");
    client.println("      const ledSize = 16;");
    client.println("      for (let i = 0; i < MATRIX_WIDTH * MATRIX_HEIGHT; i++) {");
    client.println("        const x = i % MATRIX_WIDTH;");
    client.println("        const y = Math.floor(i / MATRIX_WIDTH);");
    ...
    client.println("    if (!stripRealtimeInterval) { stripRealtimeInterval = setInterval(updateStripLeds, 333); }");
    client.println("  } else {");
    ...
    client.println("    if (!matrixRealtimeInterval) { matrixRealtimeInterval = setInterval(updateMatrixLeds, 333); }");
    client.println("  } else {");
